...
<form id="pagerForm" method="post" action="${ctx}/tbwork">
	 <input type="hidden" name="pageNum" value="${query.pageNum}" /><!--【必须】value=1可以写死-->
      ...
      <input type="hidden" name="orderDirection" value="${query.orderDirection}" /><!--【可选】升序降序-->
				<input type="hidden"  value="${query.workName}" name="workName" />
	...
	<form onsubmit="return navTabSearch(this);" action="${ctx}/tbwork" method="post">
		...
		<tr>	
            ...
            <div class="box_bottom pb5 pt5 pr10" style="border-top:1px solid #dadada;">
			  ...
			  <!--
				 ...
				 -->
				 <input type="button" class="ext_btn ext_btn_error" value="批量删除" onclick="batchDelete('${ctx}/tbwork','items')">
                 <input type="button" class="ext_btn ext_btn_success" value="新增" onclick="openEditDialog('新增','${ctx}/tbwork/new','600px','300px')">
              </div>
            </div>
          </div>
        </div>
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="list_table">
		<thead>
			  <tr>
				...
				<th orderField="workName" >岗位名称</th>
				<th orderField="workName" >操作</th>
			  </tr>
		  </thead>
		<tbody>
			 <c:forEach items="${page.result}" var="item" varStatus="status">
				...
				<td><input type="checkbox" name="items" value="${item.workId}"></td>
				<td>${item.workName}&nbsp;</td>
				<td><a href="javascript:openEditDialog('编辑','${ctx}/tbwork/${item.workId}/edit','800px','400px');">修改</a>&nbsp;</td>
			  </tr>		  
		  	 </c:forEach>
		</tbody>
	</table>
